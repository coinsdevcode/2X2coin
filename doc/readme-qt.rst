...
Alternatively, install Qt Creator and open the `2X2-qt.pro` file.

An executable named `2X2-qt` will be built.
...
```
After compiling the 2X2d daemon:
cd ..
...
To use UPnP for port forwarding behind a NAT router (recommended, as more connections overall allow for a faster and more stable 2X2 experience), pass the following argument to qmake:
...
A warning for people using the *static binary* version of 2X2 on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of 2X2 is linked against libdb 5.0 (see also `this Debian issue`_).
...
installed causes 2X2-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
